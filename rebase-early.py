...
def get_arrow_addr_list_arm(filename,area_length):
	...
	for match in matchs:
		r=match.group()
		temp=(r[1]<<8)+r[0]+0x08+match.start()
...
arrow_function={"arc":get_arrow_addr_list_arc,"arm":get_arrow_addr_list_arm,"mips":get_arrow_addr_list_mips,"powerpc":get_arrow_addr_list_powerpc,"sparc":get_arrow_addr_list_sparc,"superh":get_arrow_addr_list_superh,"x86":get_arrow_addr_list_x86} 

if platform not in arrow_function:
	
	print "Error:platform not support!"

	print "only support platform:arc,arm,mips,powerpc,sparc,superh,x86"
...
area_length=target_addr_list[-1]-target_addr_list[0]
